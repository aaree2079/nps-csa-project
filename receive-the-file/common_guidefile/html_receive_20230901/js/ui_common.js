...
const commonUI = {
	...
	gnav: {
		...
		navOpenAll: function(){
			...
			this.btnMenu.querySelector('span').textContent = '닫기';
		},
		navCloseAll: function(){
			...
			this.btnMenu.querySelector('span').textContent = '열기';
		},
	},
	mobileNav:{
		init: function () {
		...
		_initEvents: function () {
			...
			this.btnMenu.addEventListener('click', (e)=>{
				e.target.classList.contains('active') ? this.navClose() : this.navOpen();
			});
	...
	sideNav:{
		...
		_initElements: function () {
			...
			this.dep1 = document.querySelectorAll('.side-nav .snav-dep1 > li > a');
			this.dep2 = document.querySelectorAll('.side-nav .snav-dep2');
